...
def __main__():
    ...
    usage = usage + "    python3 %prog -i Discussions.txt -o docs/tech/posts"
    usage = usage + "\n\nDescription:\n"
    ...
    parser.add_option("-o", "--outdir", dest="outdir", default='docs/tech/posts', help="Output directory (%default).")
    ...
    if 'nodes' in discussionsDict.keys():
        ...
        for discussion in discussionsList:
            ...
            if not discussion_category.startswith('1.'):
                continue
       
            discussion_category_name = discussion_category.split('-')[-1]
            ...
            # 处理输出结果
            savedPostDir = discussion_category
